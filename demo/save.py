...
def register():
    ...
    if form.validate_on_submit():
        with sqlite3.connect("database.db") as con:
            cur = con.cursor()
        cur.execute("INSERT INTO user(username,email,password,confirm,Age,Location) VALUES (?,?,?,?,?,?)", (form.username.data,
                    form.email.data, form.password.data, form.confirm_password.data, form.Age.data, form.Location.data))
        con.commit()
...
def recommend(user_id):
    ...
    books = pd.read_csv('REdemo_v3.1/data/books.csv', sep=';',
                        error_bad_lines=False, encoding="latin-1", dtype=object)
    books.columns = ['ISBN', 'bookTitle',
                     'bookAuthor', 'yearOfPublication', 'publisher']
    users = pd.read_csv('REdemo_v3.1/data/users.csv', sep=';',
                        error_bad_lines=False, encoding="latin-1")
    users.columns = ['userID', 'Location', 'Age']
    ratings = pd.read_csv('REdemo_v3.1/data/ratings.csv', sep=';',
                          error_bad_lines=False, encoding="latin-1", dtype=object)
    ...
    print('The sparsity level of Book Crossing dataset is ' +
          str(sparsity*100) + ' %')
    ...
    def new_user_recommendation():
        ...
        top10 = ratings_count.sort_values(
            'bookRating', ascending=False).head(3)
...
# ---------------------------------------Discount end_________________________________________
    ...
    print("-----------------------=================-------------------------------------=", list2)
    # ====================================================================
    print("aaaaaaaaaaaaaaa", predict1[0][0],
          "aaaaaaaaaaaaaaa", predict22[0][0])
